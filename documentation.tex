...
Sensor data is exposed through a Raspberry Pi endpoint and consumed by the assistant through two paths: WebSocket push streaming for primary real-time updates and HTTP polling fallback for compatibility. This architecture decouples hardware sampling from assistant inference while preserving near-real-time control signals.
...
The orientation field is monitored continuously. In the current implementation, the assistant first attempts to receive pushed JSON frames over WebSocket and falls back to HTTP endpoint polling if needed. Valid values are normalized and mapped to educational modes. This mapping drives spoken announcements and game-rule guidance.
...
\begin{enumerate}
    \item Receive gyro payload from WebSocket stream, or poll HTTP endpoint as fallback.
...
The gyro transport worker runs in a daemon thread and continues operation across transient endpoint failures. In WebSocket mode, the client reconnects automatically on connection loss; in HTTP mode, polling retries continue based on configured intervals. This approach improves robustness for real-world sensor intermittency.
...
\begin{itemize}
    ...
    \item Gyro parameters: WebSocket URL, HTTP URL fallback, polling interval, request timeout.
...
def next_orientation(payload, last_orientation):
    orientation = normalize(payload.get("gyro_orientation"))
...
# Transport strategy:
# 1) Try WebSocket stream from VA_GYRO_WS_URL
# 2) Fallback to HTTP polling via VA_GYRO_URL
